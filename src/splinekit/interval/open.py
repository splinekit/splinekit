...
open intervals between a pair of finite real numbers.
"""
...
class Open (
    ...
    def __repr__ (
        ...
        return f"Open(({self._leftbound}, {self._rightbound}))"
    ...
    def __new__ (
        ...
        r"""
        Creates the bounded open interval
        :math:`\{x\in{\mathbb{R}}|x_{0}<x<x_{1}\}=(x_{0},x_{1})` that
        contains all numbers that are larger than :math:`x_{0}` and smaller
        than :math:`x_{1}.`
        ...
        -------
        Interval
            An ``Open`` interval if ``bounds`` contains two finite
